...
Protected Class AppleIntrospection
	...
	#tag Method, Flags = &h0
		Function Ivar(position as uinteger) As AppleIvar
		  ...
		  if myivars <> NIL then
		    return new AppleIvar (myivars(position*IntegerSize))
		  end if
		End Function
	#tag EndMethod
	...
	#tag Method, Flags = &h0
		Function Method(position as uinteger) As AppleMethod
		  ...
		  if mymethods <> NIL then
		    return new AppleMethod (mymethods(position * IntegerSize))
		  end if
		End Function
	#tag EndMethod
	...
	#tag Method, Flags = &h0
		Function PropertyNames() As text()
		  ...
		  if myprops <> NIL then
		    for q as uinteger = 0 to myprops.Ubound
		      dim myprop as AppleProperty = property_ (q)
	...
	#tag Method, Flags = &h0
		Function Property_(position as uinteger) As AppleProperty
		  ...
		  if myProps <> NIL then
		    return new AppleProperty (myProps(position * IntegerSize))
		  end if
		End Function
	#tag EndMethod
